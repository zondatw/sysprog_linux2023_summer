...
// 初始化 mpmc 的 queue ，設定 enq_barrier 和 deq_barrier 的 size
...
static void *mpmc_find_cell(node_t *volatile *ptr, long i, handle_t *th)
{
    ...
    for (long j = curr->id; j < i / N; ++j) {
        ...
        if (!next) { /* start filling */
            ...
            // TODO: tmp->id = ZBBB; /* next node's id */
...
void *mpmc_dequeue(mpmc_t *q, handle_t *th)
{
    ...
    // TODO: long index = __atomic_fetch_add(&q->pop_index, ZCCC,
    // __ATOMIC_SEQ_CST);
...
// 等待 prod_barrier 當所有 producer 都啟動後，將 task 放進 queue 中
...
#define N_THREADS 4
static bool *array;
// 等待 cons_barrier 當所有 consumer 都啟動後，將 task 從 queue
// 中取出，並將指定的 array index 改為 true
...
int main(int argc, char *argv[])
{
    ...
    // 剛開始先初始化 prod_barrier 和 cons_barrier，設定為 N_HTREADS + 1
    ...
    fflush(stdout);
    // 建立 (1 + N_THREADS * COUNTS_PER_THREAD) * sizeof(bool) 大小的 array
    ...
    // 產生 N_THREADS 個 producer 和 consumer
    ...
    for (int i = 0; i < 8; i++) {
        ...
        // 先等待兩次 prod_barrier 再等待一次 cons_barrier
